...
class TabPageGnkt(TabPageUnion):
        ...
        try:
            if previus_well:
                ...
                params = {
                    ...
                    "date_repair": previus_well.split(" ")[2]
                }
                result_gnkt = ApiClient.request_params_get(ApiClient.find_gnkt_data_by_well_number(), params)
                if result_gnkt:
                    self.length_gnkt_edit.setText(str(result_gnkt["length_gnkt"]))
                    self.iznos_gnkt_edit.setText(str(result_gnkt["wear_gnkt"]))
                    self.pipe_mileage_edit.setText(str(result_gnkt["mileage_gnkt"]))
                    self.pvo_number_edit.setText(str(result_gnkt["pvo_number"]))
    ...
    def update_number_gnkt(self, gnkt_number):
        if gnkt_number != '':
            well_previus_list = self.append_list_previus_well(gnkt_number)
            self.previous_well_combo.clear()
            self.previous_well_combo.addItems(list(map(str, well_previus_list)))

    def append_list_previus_well(self, gnkt_number):
...
class GnktModel(WindowUnion):
            ...
            for col, value in enumerate(row):
                if 'Ойл' in data_list.contractor:
                    if 'А.Р. Хасаншин' in str(value):
                        ...
                        self.insert_image(ws2, f'{data_list.path_image}imageFiles/Галиев.png', coordinate)
    ...
    def work_opz_gnkt(self, acid_info):
        ...
        opz.insert(0, [None, None,
                       f"Исследовать скважину на приёмистость по малому затрубному пространству при "
                       f"Рзак={self.data_well.expected_pressure}атм с составлением акта в присутствии "
                       f"представителя ЦДНГ с составлением соответствующего акта (для вызова представителя давать "
                       f"телефонограмму в ЦДНГ). Определение приёмистости производить после насыщения пласта "
                       f"не менее 6м3 или при установившемся давлении закачки, но не более 1 часов.",
                       None, None, None, None, None, None, None,
    ...
    def schema_well(self, current_bottom_edit, fluid_edit, gnkt_number_combo,
        ...
        if self.data_well.curator == 'ОР' and self.data_well.region == 'ТГМ' and \
                self.data_well.depth_fond_paker_second_before["before"] != 0:
            length_paker = self.data_well.depth_fond_paker_second_before["before"] - \
                           self.data_well.depth_fond_paker_before["before"]
            if length_paker > 4:
        ...
        schema_well_list = [
            ...
            [None, None, None, None, None, None, None, None, None, nkt_schema, None, None, 'Направление', None,
             ...
             round(
                 self.data_well.column_direction_diameter.get_value - 2 * self.data_well.column_direction_wall_thickness.get_value,
                 1),
            ...
            [None, None, None, None, None, None, None, None, None, None, None, None, 'Экспл. колонна', None, None,
             ...
             f'0-', self.data_well.shoe_column.get_value,
             f'{self.data_well.level_cement_column.get_value}-{self.data_well.shoe_column.get_value}',
             volume_pm_ek,
             self.well_volume_ek],
            [None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None,
             None, None, None, "", ""],
        ...
        try:
            ...
            for plast in sorted(self.data_well.plast_all, key=lambda x: self.get_start_depth(
                ...
                for index, interval in enumerate(self.data_well.dict_perforation[plast]['интервал']):
                    ...
                    pvr_list.append(
                        [None, None, None, None, None, None, None, None, None, None, None, None, plast, None,
                         vertikal_1,
                         None, interval[0],
