...
class TubingPressureWindow(WindowUnion):
    ...
    def add_work(self):
        try:
            ...
            self.distance_between_nkt = self.tab_widget.currentWidget().distance_between_nkt_edit.text().replace(',', '.')
            if self.distance_between_nkt != '':
                self.distance_between_nkt = int(float(self.distance_between_nkt))
            self.select_nkt_combo = self.tab_widget.currentWidget().select_nkt_combo.currentText()
            self.current_bottom = int(float(self.tab_widget.currentWidget().current_edit.text().replace(',', '.')))
            ...
            self.length_nkt = self.tab_widget.currentWidget().length_nkt_edit.text().replace(',', '.')
            if self.length_nkt != '':
                self.length_nkt = int(float(self.length_nkt))
            self.pressure_data = self.tab_widget.currentWidget().pressure_edit.text().replace(',', '.')
        ...
        if self.select_nkt_combo != 'Фондовые НКТ' or self.data_well.curator == 'ОР':
            block_pack_list = [
                ...
                [None, None,
                 ...
                 None, None, None,
                 'Мастер КРС', lifting_nkt_norm(self.length_nkt, 1)],
            ]
        else:
