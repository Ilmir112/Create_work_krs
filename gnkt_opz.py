...
def gnkt_work(self):
    ...
    на {CreatePZ.max_admissible_pressure}атм. Опрессовать  выкидную линию от устья скважины до желобной ёмкости на {round(CreatePZ.max_admissible_pressure*1.5,1)}атм (надёжно закрепить, оборудовать \    дроссельными задвижками)',
         None, None, None, None, None, None, None,
    ...
    устье скважины на Р={CreatePZ.max_admissible_pressure}атм с выдержкой 30 мин (опрессовку ПВО зафиксировать в вахтовом журнале). Оформить соответствующий акт\
    ...
    [None, 12, f'Произвести спуск БДТ + насадка 5 каналов до {CreatePZ.current_bottom}м (забой) с промывкой скважины мин.водой уд.веса {fluid_work}  \
    ...
    [None, 13, f'Промыть НКТ и скважину с гл.{CreatePZ.current_bottom}м мин.водой уд.веса {fluid_work}  в 1 цикл с добавлением 1т растворителя с \
    ...
    не превышая давления закачки не более  Р={CreatePZ.max_admissible_pressure}атм. ',
        None, None, None, None, None, None, None,
    ...
    жидкости при не более Рзак={CreatePZ.max_admissible_pressure}атм при установленном герметичном пакере. Операции по задавке и изливу произвести 3-4 раза \
    ...
    opz = [[None, 16, f'Допустить БДТ до забоя. Промыть скважину  мин.водой уд.веса {fluid_work}  с составлением соответствующего акта. \
    ...
    не более {CreatePZ.max_admissible_pressure}атм  (перед определением приемистости произвести закачку тех.воды не менее 6м3 или при установившемся давлении \
    закачки, но не более 1 часа). Установить БДТ на гл.{CreatePZ.current_bottom}м.',
         None, None, None, None, None, None, None,
    ...
    вдоль интервалов перфорации {min(list(CreatePZ.work_pervorations_dict.keys()))}-{max(list(CreatePZ.work_pervorations_dict.values()))}м\
     ...
     давления закачки не более Р={CreatePZ.max_admissible_pressure}атм.\n(для приготовления соляной кислоты в объеме {acid_V}м3 - {acid_pr}%\
      необходимо замешать {round(acid_V*acid_pr/24*1.118,1)}т HCL 24% \
     и пресной воды {round(acid_V-acid_V*acid_pr/24*1.118,1)}м3) ',
         None, None, None, None, None, None, None,
    ...
    [None, 18, f'Закачку 2,2м3 кислоты производить при открытом малом затрубном пространстве на циркуляции. Закачку оставшейся \
        кислоты в объеме {acid_V-2.2}м3 производить при закрытом затрубном пространстве. Составить Акт.',
         None, None, None, None, None, None, None,
            'Мастер ГНКТ, состав бригады', None],
    [None, 19, f'Продавить кислоту в пласт мин.водой уд.веса {fluid_work} в объёме 3м3 при давлении не более {CreatePZ.max_admissible_pressure}атм. Составить Акт',
        None, None, None, None, None, None, None,
