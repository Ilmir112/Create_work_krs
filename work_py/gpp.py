...
class GppWindow(WindowUnion):
    ...
    def grp_gpp_work(self, gpp_depth, current_depth, diameter_paker, gis_otz_after_true_quest):
        ...
        if gpp_300 is False:
            QMessageBox.warning(self, 'Ошибка', 'При проведении Работ ГПП  в компоновку ГРП добавляется пакер и '
                                                'сажается на глубину 300м, необходимо добавить интервал '
                                                'скреперования 280-320м')
            return
