...
class CreatePZ(MyMainWindow):
    ...
    #                          'gnkt_frez']:
    #         if work_plan != 'plan_change':
    #             for row_ind, row in enumerate(ws.iter_rows(values_only=True, max_col=16)):
    #                 ws.row_dimensions[row_ind].hidden = False
    #                 if 'ПЛАН РАБОТ' == row[1] \
    #                         and work_plan == 'dop_plan':
    #                     ws.cell(row=row_ind + 1, column=2).value = \
    #                         f'ДОПОЛНИТЕЛЬНЫЙ ПЛАН РАБОТ № {self.data_well.number_dp}'
    #
    #                 if 'XI. ПЛАН РАБОТ:' == row[1] \
    #                         and work_plan == 'dop_plan':
    #                     ws.cell(row=row_ind + 1, column=2).value = \
    #                         f'Техническое задание от ЦДНГ'
    #
    ...
    #             ws.row_dimensions[i + 1].height = 20
    #
    #             self.insert_events_gnvp(ws, dict_events_gnvp)
    #
    #             self.data_well.insert_index += len(dict_events_gnvp[work_plan]) - 1
    #
    #             ws.row_dimensions[2].height = 30
    ...
    #             self.data_well.insert_index = ws2.max_row
    #             self.copy_data_excel_in_excel(
    #                 ws, ws2, self.data_well.cat_well_min.get_value, self.data_well.data_well_max.get_value + 7)
    #             self.copy_data_excel_in_excel(
    #                 ws, ws2, self.data_well.data_fond_min.get_value, self.data_well.condition_of_wells.get_value + 2)
    #
    #             self.insert_events_gnvp(ws2, dict_events_gnvp, 2)
    #
    #             self.copy_data_excel_in_excel(
    #                 ws, ws2, self.data_well.data_x_min.get_value, self.data_well.data_x_min.get_value + 2)
    #
    #             # self.copy_data_excel_in_excel(
    #             #     ws, ws2, self.data_well.prs_copy_index.get_value, self.data_well.data_fond_min.get_value)
    #
    #             self.data_well.insert_index = ws2.max_row
    #             if work_plan not in ['application_pvr', 'gnkt_frez', 'gnkt_after_grp', 'gnkt_opz', 'gnkt_bopz',
    #                                  'plan_change']:
    #                 work_list = [
    #                     [None, None, 'Порядок работы', None, None, None, None, None, None, None, None, None],
    #                     [None, 'п/п', 'Наименование работ', None, None, None, None, None, None, None, None, None,
    #                      'Ответственный',
    #                      'Нормы времени \n мин/час.']]
    #
    #                 for i in range(1, len(work_list) + 1):  # Добавление показатели после ремонта
