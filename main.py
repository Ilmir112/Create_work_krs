    ...
    def insert_data_in_database(self, excel_data_dict, data_well):
        ...
        if data_list.connect_in_base:
            ...
            params = {
                "id": 2,
                "category_dict": {
                    "данные по категории": json.dumps(
                        self.data_well.dict_category, ensure_ascii=False
                    )
                },
                "type_kr": self.data_well.type_kr.split(" ")[0],
                "work_plan": string_work,
                "excel_json": {
                    "excel": json.dumps(excel_data_dict, ensure_ascii=False)
                },
                "data_change_paragraph": {
                    "данные": json.dumps(self.data_well.data_list)
                },
