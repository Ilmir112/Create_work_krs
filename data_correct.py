...
class TabPageSoCorrect(TabPageUnion):
    ...
    def __init__(self, data_well: FindIndexPZ):
        ...
        self.distance_from_well_to_sampling_point_line = FloatLineEdit()
        self.distance_from_well_to_sampling_point_line.setText(
            str(self.data_well.distance_from_well_to_sampling_point)
        )
        ...
        self.column_type.setText(
            f"{self.ifNone(self.data_well.column_diameter.get_value)}"
        )
        ...
        self.column_wall_thickness_edit_type2.setText(
            f"{str(self.ifNone(self.data_well.column_wall_thickness.get_value)).strip().replace('.', ',')}"
        )
        ...
        self.shoe_column_edit_type2.setText(
            f"{self.ifNone(self.data_well.shoe_column.get_value)}"
        )
        ...
        if "-" in str(self.data_well.level_cement_column.get_value):
            self.level_cement_edit.setText(
                self.ifNone(self.data_well.level_cement_column.get_value)
                .split("-")[0]
                .strip()
            )
        else:
            self.level_cement_edit.setText(
                f"{self.ifNone(self.data_well.level_cement_column.get_value)}"
            )
        ...
        self.column_add_edit_type.setText(
            f"{self.ifNone(self.data_well.column_additional_diameter.get_value)}"
        )
        ...
        self.head_column_add_edit_type2.setText(
            f"{self.ifNone(self.data_well.head_column_additional.get_value)}"
        )

        self.shoe_column_add_label = QLabel("башмак \nдоп колонны", self)
        self.shoe_column_add_edit_type2 = FloatLineEdit()
        self.shoe_column_add_edit_type2.setText(
            f"{self.ifNone(self.data_well.shoe_column_additional.get_value)}"
        )
        ...
        self.paker2_depth_do_edit_type.setText(
            self.remove_non_numeric_chars(
                self.ifNone(
                    str(self.data_well.depth_fond_paker_second_before["before"])
                )
            )
        )
        ...
        self.paker2_depth_posle_edit_type.setText(
            self.remove_non_numeric_chars(
                self.ifNone(str(self.data_well.depth_fond_paker_second_before["after"]))
            )
        )
        ...
        if isinstance(self.data_well.date_commissioning.get_value, datetime):
            self.data_well.date_commissioning = data_list.ProtectedIsNonNone(
                self.data_well.date_commissioning.get_value.strftime("%d.%m.%Y"))
        ...
        if isinstance(self.data_well.result_pressure_date.get_value, datetime):
            self.data_well.result_pressure_date = data_list.ProtectedIsNonNone(
                self.data_well.result_pressure_date.get_value.strftime("%d.%m.%Y"))
        ...
        if len(self.dict_sucker_rod_po) != 0:
            ...
            for sucker_po, length_po in self.dict_sucker_rod_po.items():
                ...
                self.labels_sucker_po[n] = (
                    sucker_rod_po_line_edit,
                    length_sucker_po_line_edit,
                )
                n += 1
        else:
            sucker_rod_po_line_edit = QLineEdit(self)
    ...
    def ifNone(self, string):
        try:
            ...
            if "/" in str(string):
                ...
                return (
                    int(float(str(string).replace(",", ".")))
                    if str(round(float(str(string).replace(",", ".")), 1))[-1] == "0"
                    else round(float(str(string).replace(",", ".")), 1)
                )
            else:
...
class DataWindow(WindowUnion):
    ...
    def add_row_table(self):
        ...
        try:
            ...
            head_column = float(
                self.current_widget.head_column_edit_type2.text().replace(",", ".")
            )
            column_add_True = str(
                self.current_widget.column_add_true_comboBox.currentText()
            )
            ...
            if self.check_date_format(result_pressure_date) is False:
                if curator != "ВНС":
                    QMessageBox.warning(
                        self,
                        "Ошибка",
                        "Не корректна дата последней опрессовки, "
            ...
            if self.check_date_format(date_commissioning_line) is False:
                if curator != "ВНС":
                    QMessageBox.warning(
                        self,
                        "Ошибка",
                        "Не корректна дата ввода в эскплуатацию, "
                        'ожидается дата в формате "dd.mm.yyyy',
                    )
                    return
                else:
            ...
            date_commissioning_line = datetime.strptime(
                date_commissioning_line, "%d.%m.%Y"
            )
            ...
            if curator == "ОР":
                ...
                expected_pressure_edit = (
                    self.current_widget.expected_pressure_edit.text()
                )
            else:
            ...
            if self.current_widget.labels_nkt:
                for key, value in self.current_widget.labels_nkt.values():
                    if key.text() != "" and value.text() != "":
                        self.data_well.dict_nkt_before[key.text()] = self.check_if_none(
                            float(value.text())
                        )
            ...
            if self.current_widget.labels_nkt_po:
                for key, value in self.current_widget.labels_nkt_po.values():
                    if key.text() != "" and value.text() != "":
                        self.data_well.dict_nkt_after[key.text()] = self.check_if_none(
                            float(value.text())
                        )

            if len(self.data_well.dict_nkt_before) == 0:
                mes = QMessageBox.question(
                    self, "Расчет на ГНО", "В скважине НКТ отсутствует?"
                )
            ...
            if self.current_widget.labels_sucker:
                for key, value in self.current_widget.labels_sucker.values():
                    if key.text() != "" and value.text() != "":
                        self.data_well.dict_sucker_rod[key.text()] = self.check_if_none(
                            int(float(value.text()))
                        )

            if self.current_widget.labels_sucker_po:
                for key, value in self.current_widget.labels_sucker_po.values():
                    if key.text() != "" and value.text() != "":
                        self.data_well.dict_sucker_rod_after[key.text()] = (
                            self.check_if_none(int(float(value.text())))
                        )

            question = QMessageBox.question(
                self, "выбор куратора", f"Куратор ремонта сектор {curator}, верно ли?"
            )
            ...
            if any(
                ...
                QMessageBox.information(
                    self,
                    "Внимание",
                    "Не все поля в данных колонне соответствуют значениям",
                )
                close_file = False
            ...
            if (
                ...
                QMessageBox.information(
                    self,
                    "Внимание",
                    "Не все поля в доп колонне соответствуют значениям",
                )
                close_file = False
            ...
            if self.if_none(level_cement) is False:
                QMessageBox.information(
                    self, "Внимание", "Уровень цемента за ЭК не соответствуют значениям"
                )
                close_file = False
            if (
                ...
                QMessageBox.information(
                    self,
                    "Внимание",
                    "Не все поля в Направлении соответствуют значениям",
                )
                close_file = False
            if (
            ...
            if (
                ...
                QMessageBox.information(
                    self,
                    "Внимание",
                    "Не все поля в даннахы по насосом соответствуют значениям",
                )
                close_file = False
            if isinstance(self.if_none(head_column_additional), str):
                ...
                if (
                    ...
                    QMessageBox.information(
                        self, "Внимание", "В скважине отсутствует доп колонна"
                    )
                    close_file = False
                else:
                    QMessageBox.information(
            ...
            if self.data_well.column_additional:
                ...
                if all(
                    ...
                    if int(float(column_additional_diameter)) >= float(column_type):
                        QMessageBox.information(
                            self, "Внимание", "Ошибка в диаметре доп колонны"
                        )
                        close_file = False
                    ...
                    if int(float(str(head_column_additional).replace(",", "."))) < 10:
                        msg = QMessageBox.question(
                            self, "Внимание", "доп колонна начинается с устья?"
                        )
            ...
            try:
                ...
                QMessageBox.warning(
                    self,
                    "Ошибка",
                    f"Башмак ЭК не корректен {type(e).__name__}\n\n{str(e)}",
                )

            if close_file is False:
                return
            elif close_file is True:
                self.data_well.column_diameter = ProtectedIsDigit(
                    self.check_if_none(column_type)
                )
                self.data_well.column_wall_thickness = ProtectedIsDigit(
                    self.check_if_none(column_wall_thickness)
                )
                self.data_well.shoe_column = ProtectedIsDigit(
                    float(self.check_if_none(shoe_column))
                )
                ...
                self.data_well.bottom_hole_drill = ProtectedIsDigit(
                    self.check_if_none(bottomhole_drill)
                )
                self.data_well.bottom_hole_artificial = ProtectedIsDigit(
                    self.check_if_none(bottomhole_artificial)
                )
                ...
                self.data_well.level_cement_column = ProtectedIsDigit(
                    self.check_if_none(level_cement)
                )
                self.data_well.max_angle = ProtectedIsDigit(
                    self.check_if_none(max_angle)
                )
                self.data_well.max_expected_pressure = ProtectedIsDigit(
                    self.check_if_none(max_expected_pressure)
                )
                self.data_well.max_admissible_pressure = ProtectedIsDigit(
                    self.check_if_none(max_admissible_pressure)
                )
                self.data_well.date_commissioning = ProtectedIsNonNone(
                    date_commissioning_line
                )
                self.data_well.result_pressure_date = ProtectedIsNonNone(
                    result_pressure_date
                )

                # print(f'макс {self.data_well.max_expected_pressure.get_value}')
                self.data_well.dict_pump_shgn["before"] = self.check_if_none(
                    dict_pump_shgn_do
                )
                self.data_well.dict_pump_shgn_depth["before"] = self.check_if_none(
                    dict_pump_shgn_h_do
                )
                self.data_well.dict_pump_shgn_depth["after"] = self.check_if_none(
                    dict_pump_shgn_h_posle
                )
                self.data_well.dict_pump_shgn["after"] = self.check_if_none(
                    dict_pump_shgn_posle
                )

                self.data_well.dict_pump_ecn["before"] = self.check_if_none(
                    dict_pump_ecn_do
                )
                self.data_well.dict_pump_ecn_depth["before"] = self.check_if_none(
                    dict_pump_ecn_h_do
                )
                self.data_well.dict_pump_ecn["after"] = self.check_if_none(
                    dict_pump_ecn_posle
                )
                self.data_well.dict_pump_ecn_depth["after"] = self.check_if_none(
                    dict_pump_ecn_h_posle
                )

                self.data_well.paker_before["before"] = self.check_if_none(paker_do)
                self.data_well.depth_fond_paker_before["before"] = self.check_if_none(
                    depth_fond_paker_do
                )
                self.data_well.paker_before["after"] = self.check_if_none(paker_posle)
                self.data_well.depth_fond_paker_before["after"] = self.check_if_none(
                    depth_fond_paker_posle
                )

                self.data_well.paker_second_before["before"] = self.check_if_none(
                    paker2_do
                )
                self.data_well.depth_fond_paker_second_before["before"] = (
                    self.check_if_none(depth_fond_paker2_do)
                )
                self.data_well.paker_second_before["after"] = self.check_if_none(
                    paker2_posle
                )
                self.data_well.depth_fond_paker_second_before["after"] = (
                    self.check_if_none(depth_fond_paker2_posle)
                )
                self.data_well.static_level = ProtectedIsDigit(
                    self.check_if_none(static_level)
                )
                self.data_well.dinamic_level = ProtectedIsDigit(
                    self.check_if_none(dinamic_level)
                )
                ...
                self.data_well.level_cement_direction = ProtectedIsDigit(
                    self.check_if_none(level_cement_direction)
                )
                ...
                self.data_well.level_cement_conductor = ProtectedIsDigit(
                    self.check_if_none(level_cement_conductor)
                )
                if curator == "ОР":
                    self.data_well.expected_pressure = self.check_if_none(
                        expected_pressure_edit
                    )
                    self.data_well.expected_pickup = self.check_if_none(
                        expected_pickup_edit
                    )
                    self.data_well.expected_pick_up[self.data_well.expected_pickup] = (
                        self.data_well.expected_pressure
                    )
                    ...
                    self.data_well.percent_water = int(
                        self.check_if_none(proc_water_edit)
                    )
                ...
                elif curator in ["ГРР"] and self.data_well.work_plan in [
                    "gnkt_after_grp"
                ]:
                    self.data_well.bvo = True
        ...
        if str(self.data_well.paker_before["after"]).lower() not in [
            "0",
            0,
            "-",
            "отсут",
            ...
            try:
                paker_diameter = TabPageSo.paker_diameter_select(
                    self, float(self.data_well.depth_fond_paker_before["after"])
                )
                ...
                QMessageBox.information(
                    self,
                    "Ошибка обработки",
                    f"ошибка проверки ПЗ в части соответствия "
                    f"диаметра пакера \n {type(e).__name__}\n\n{str(e)}",
                )
                return
