...
def gnkt_work(self):
    ...
    if 2 in CreatePZ.cat_H2S_list or '2' in CreatePZ.cat_H2S_list:
        fluid_work = f'{fluid_work_insert}г/см3 с добавлением поглотителя сероводорода ХИМТЕХНО ' \
                     f'101 Марка А из' \
    ...
    [None, 13, f'При закрытой центральной задвижке фондовой арматуры опрессовать ГНКТ и все '
               f'нагнетательные линии на {round(CreatePZ.max_expected_pressure*1.5,1)}атм. '
               ...
               f'опрессовать на {CreatePZ.max_expected_pressure}атм с выдержкой 30мин. Опрессовку ПВО зафиксировать в вахтовом журнале. Установить на малом и большом затрубе технологический манометр. Провести УТЗ и инструктаж. Опрессовку проводить в присутствии мастера, бурильщика, машиниста подъемника и представителя супервайзерской службы. Получить разрешение на проведение работ.',
        None, None, None, None, None, None, None,
    ...
    [None, 23, f'ВНИМАНИЕ: При получении посадки до гл. {CreatePZ.H_F_paker_do["do"]}м и наличии разгрузки на промывочный '
               ...
               f'более 5м/мин. Внимание: после промывки НКТ до гл.2087м приподнять ГТ на 20м выше пакера и '
               f'промыться до выхода чистой тех. жидкости (без мех.примесей) и '
               f'только после этого продолжить промывку ниже пакера', 1],
...
[None, 29, f'Последующие 2 часа освоения производить на выбранном оптимальном режиме освоения '
     ...
     'Мастер ГНКТ', None],
        ...
        [None, 29, f'Не допускать увеличение нагрузки на г/трубу в процессе спуска. РАЗГРУЗКА Г/ТРУБЫ НЕ БОЛЕЕ 500 кг от собственного веса на этой глубине.',
         None, None, None, None, None, None, None,
         'Мастер ГНКТ', None],
        [None, 29,
