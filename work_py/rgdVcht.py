...
def rgdWithoutPaker(self):
    rgd_list = [
        ...
        [f'РГД по колонне при закачке не менее {GnoDescentWindow.PzakPriGis(self)}атм',
         None,
         f'Произвести запись по техкарте 2.3.1: Определение профиля приемистости скважины и оценка технического'
         f' состояния '
...
def rgdWithPaker(self):
    vcht_rgd = [[''.join(['ОВТР 10ч' if well_data.region != 'ЧГМ' else 'ОВТР 4ч']),
                ...
                [None, None,
                 ...
                 'мастер КРС', None],
                [f'ГИС РГД', None,
                 f'Произвести запись по техкарте 2.3.2: определение профиля приемистости и оценку технического '
                 f'состояния '
