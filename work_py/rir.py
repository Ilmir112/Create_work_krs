...
def rir_rpp(self):
    ...
    rir_work_list = [[f'СПО РПП до глубины {rpkDepth}м', None,
                   f'Спустить   пакер глухой {rpk_nkt(self, rpkDepth)}  на тНКТ{nkt_diam}мм до глубины {rpkDepth}м '
    ...
    'мастер КРС', descentNKT_norm(rpkDepth,1.2)],
     ...
     [f'установка пакера  на  {rpkDepth}м', None,
...
def rir_rpk(self):
      ...
      [f'посадить пакер на глубину {rpkDepth}м'
          , None,
                   f'посадить пакер на глубину {rpkDepth}м',
                    None, None, None, None, None, None, None,
                    'мастер КРС', 1],
      [f'Насыщение 5м3. Определить приемистость {plast} при Р=80-100атм',
       None,
        ...
        rir_q_list = [
                      [f'Насыщение 5м3. Определить Q {plast} при Р=80-100атм',
                       None,
                       f'Произвести насыщение скважины в объеме 5м3. Определить приемистость {plast} при Р=80-100атм '
                       f'в присутствии представителя УСРСиСТ или подрядчика по РИР. (Вести контроль за отдачей жидкости '
                       f'после закачки, объем согласовать с подрядчиком по РИР). В случае приёмистости менее  250м3/сут '
    ...
    rir_work_list = [[f'СПО пакера РПК до глубины {rpkDepth}м', None,
                   f'Спустить   пакера РПК {rpk_nkt(self, rpkDepth)}  на тНКТ{nkt_diam}мм до глубины {rpkDepth}м с '
    ...
    'мастер КРС', descentNKT_norm(rpkDepth,1.2)],
     ...
     [f'РИР {plast} с установкой пакера РПК на глубине {rpkDepth}м ', None,
...
def rirWithPero(self):
    ...
    uzmPero_list = [
        ...
        [f'УЦМ в интервале {rirRoof}-{rirSole}м', None,
...
def rir_paker(self):
    ...
    rir_q_list = [f'насыщение 5м3. Определить Й {plast} при Р=80-100атм. СКВ', None,
                   f'Произвести насыщение скважины в объеме 5м3. Определить приемистость {plast} при Р=80-100атм '
                   f'в присутствии представителя УСРСиСТ или подрядчика по РИР. (Вести контроль за отдачей жидкости '
                   f'после закачки, объем согласовать с подрядчиком по РИР). В случае приёмистости менее  250м3/сут '
                   f'при Р=100атм произвести соляно-кислотную обработку скважины в объеме 1м3 HCl-12% с целью увеличения '
    ...
    rir_paker_list = [[ f'РИР c пакером {plast} c плановой кровлей на глубине {rirRoof}м',
                        None,
