...
def gnkt_work(self):
    ...
    if acid == 'HCl':
        ...
        acid_sel_short = f'СКО пласта {" ".join(CreatePZ.plast_work)}  в объеме  {acid_V}м3  ({acid} - {acid_pr} %)'
    elif acid == 'ВТ':
    ...
    [f'Промыть НКТ и скважину с гл.{CreatePZ.current_bottom}м', 13, f'Промыть НКТ и скважину с гл.{CreatePZ.current_bottom}м мин.водой уд.веса {fluid_work}  в 1 цикл'
    ...
    [f'обработку НКТ {V_rast}м3 растворителя', 14, f'Произвести  обработку НКТ {V_rast}м3 растворителя в присутствии представителя Заказчика при открытом'
    ...
    [f'гидросвабирование в инт {CreatePZ.perforation_roof}-'
               f'{CreatePZ.perforation_sole}м при Рзак={CreatePZ.max_admissible_pressure}атм', 23, f'Произвести гидросвабирование пласта в интервале {CreatePZ.perforation_roof}-'
    ...
    [f'Исследовать скважину на приёмистость при Рзак={expected_P}атм', 24, f'Исследовать скважину на приёмистость при Рзак={expected_P}атм с составлением акта в '
    ...
    opz = [[f'Допустить БДТ до забоя. Промыть скважину ', 16, f'Допустить БДТ до забоя. Промыть скважину  мин.водой уд.веса {fluid_work}  с составлением '
    ...
    [None, 18, f'Закачку {V_gntk}м3 кислоты производить при открытом малом затрубном пространстве на циркуляции. Закачку оставшейся '
               f'кислоты в объеме {round(acid_V-V_gntk, 1)}м3 производить при закрытом затрубном пространстве. Составить Акт.',
         None, None, None, None, None, None, None,
    ...
    [f'разрядку скважины для извлечения продуктов', 21, 'Произвести разрядку скважины для извлечения продуктов реакции кислоты в объёме не менее объёма закаченной кислоты '
