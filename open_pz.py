...
class CreatePZ(QMainWindow):
    ...
    def open_excel_file(self, ws, work_plan):
        ...
        from find import FindIndexPZ, Well_perforation, Well_Category
        old_index = 0

        CreatePZ.work_plan = work_plan
        well_data = FindIndexPZ(ws)


        data_x_max = well_data.cat_well_max
        print(f'индексы well {data_x_max}')
        for row_ind, row in enumerate(ws.iter_rows(values_only=True)):
            ...
            for col, value in enumerate(row):
                if not value is None and col <= 12:
                    ...
                    elif value == '4. Эксплуатационная колонна (диаметр(мм), толщина стенки(мм), ' \
                                  'глубина спуска(м))':  # Определение данных по колонне
        ...
        for row in range(CreatePZ.pipes_ind + 1, CreatePZ.condition_of_wells):
            ...
            if key != str(None) and key != '-' and "Диам" not in key:
                print(f'yrn{ws.cell(row=row, column=7).value}')
...
class FindIndexPZ(CreatePZ):
    def __init__(self, ws):
        super().__init__()
        self.ws = ws
        self.cat_well_min = ProtectedAttribute(0)
        self.cat_well_max = ProtectedAttribute()
        self.data_well_min = ProtectedAttribute()
        self.data_well_max = ProtectedAttribute()
        self.data_x_min = ProtectedAttribute()
        self.data_pvr_max = ProtectedAttribute()
        self.sucker_rod_ind = ProtectedAttribute()
        self.pipes_ind = ProtectedAttribute()
        self.data_x_max = ProtectedAttribute()
        self.condition_of_wells = ProtectedAttribute()
...
class Well_Category(FindIndexPZ):
    ...
    def __init__(self):
        super(Well_Category, self).__init__()
        self.cat_P_1 = []
        self.cat_H2S_list = []
        self.cat_gaz_f_pr = []
        self.H2S_mg = []
            ...
            for col in range(1, 13):
                ...
                if cell:
                    if 'по Pпл' in str(cell):
                        for column in range(1, 13):
                            col = self.ws.cell(row=row, column=column).value
                            # print(col)
                            if str(col) in ['1', '2', '3']:
                                self.cat_P_1.append(int(col))
                    elif 'по H2S' in str(cell) and 'по H2S' not in str(
                            self.ws.cell(row=row - 1, column=2).value):
                        for column in range(1, 13):
                            col = self.ws.cell(row=row, column=column).value
                            if str(col) in ['1', '2', '3']:
                        ...
                        cell2 = self.ws.cell(row=row, column=col - 1).value
                        if cell2:
...
#                 if not value is None and col <= 12:
#                     if 'площадь' == cell.value:  # определение номера скважины
#                         CreatePZ.well_number = row[col - 1]
#                         CreatePZ.well_area = row[col + 1]
#                     elif 'месторождение ' == value:  # определение номера скважины
#                         CreatePZ.well_oilfield = row[col + 2]
...
#                     elif 'текущий забой' in str(
#                             value).lower() and len(
#                         value) < 15:  # and any(['способ' in str(column).lower() for column in row]) == True:
#                         CreatePZ.current_bottom = row[col + 2]
#                         CreatePZ.bottom = row[col + 2]
#
#                         n = 2
...
#                         CreatePZ.column_direction_True = True
#                         print(
#                             f'направление32323 {ws.cell(row=row_ind + 1, column=col + 4).value, ws.cell(row=row_ind + 1, column=col + 4).value not in ["-", "(мм), (мм), -(м)", None]}')
#
#                         try:
#                             column_direction_data = ws.cell(row=row_ind + 1, column=col + 4).value.split('(мм),')
#                             # print(f'направление32 {column_direction_data, len(column_direction_data)}')
...
#         thread = main.ExcelWorker()
#         print(f'CreatePZ.region {CreatePZ.region, CreatePZ.well_number, CreatePZ.well_area}')
#         CreatePZ.without_damping = thread.check_well_existence(CreatePZ.well_number, CreatePZ.well_area,
#                                                                CreatePZ.region)
#
#         if len(CreatePZ.H2S_mg) == 0:
#             CreatePZ.H2S_mg = CreatePZ.H2S_mg_m3
...
#                     if str(col) in ['1', '2', '3']:
#                         CreatePZ.cat_P_1.append(int(col))
#             if 'по H2S' in str(ws.cell(row=row, column=2).value) and 'по H2S' not in str(
...
#                     [str(i).strip() is None for i in row]) == False and krs.is_number(row[2]) == True \
#                     and krs.is_number(
#                 float(str(row[2]).replace(',', '.'))) == True:  # Определение проектных интервалов перфорации
#                 if row[1] != None:
#                     CreatePZ.dict_perforation_project.setdefault(plast, {}).setdefault('вертикаль',
#                                                                                        set()).add(
#                         round(float(row[1]), 1))
