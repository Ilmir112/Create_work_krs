def raidingColumn(self):
    ...
    ryber_list = [
        ...
        [None, None, f'Собрать промывочное оборудование: вертлюг, ведущая труба (установить вставной фильтр под ведущей трубой), '
                     ...
                     f'Произвести райбирование ЭК в инт. {CreatePZ.perforation_roof-30}-{CreatePZ.perforation_sole+30}м с наращиванием, с промывкой и проработкой 5 раз каждого наращивания. '
                     ...
                     f' представителя заказчика. Допустить до текущего забоя {CreatePZ.current_bottom}м. ПРИМЕЧАНИЕ: РАСХОД РАБОЧЕЙ ЖИДКОСТИ 8-10 Л/С;'
                     f'̶  ОСЕВАЯ НАГРУЗКА НЕ БОЛЕЕ 75% ОТ ДОПУСТИМОЙ НАГРУЗКИ (УТОЧНИТЬ ПО ПАСПОРТУ ЗАВЕЗЁННОГО ГЗД И ДОЛОТА);'
                     f'̶  РАБОЧЕЕ ДАВЛЕНИЕ 4-10 МПА (УТОЧНИТЬ ПО ПАСПОРТУ ЗАВЕЗЁННОГО ВЗД);'
                     f'̶  ПРЕДУСМОТРЕТЬ КОМПЕНСАЦИЮ РЕАКТИВНОГО МОМЕНТА НА ВЕДУЩЕЙ ТРУБЕ))',
         None, None, None, None, None, None, None,
