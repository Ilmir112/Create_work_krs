...
def work_krs(self):
    ...
    lift_ecn = [
        ...
        [None, None,
         ...
         None, None, None, None, None, None, None,
         'Мастер КРС, представ заказчика', 0.3],
    ...
    lift_pump_nn = [
        ...
        [None, None,
         ...
         f'штанги. Поднять на штангах плунжер с гл. {int(CreatePZ.dict_pump_h["do"])}м с доливом тех жидкости уд.весом {fluid_work}г/см3 '
    ...
    lift_pump_nv = [
        ...
        [None, None,
         ...
         f'с гл. {int(CreatePZ.dict_pump_h["do"])}м с доливом тех жидкости уд.весом {fluid_work}г/см3 '
         f'Обеспечить не превышение расчетных нагрузок на штанговые колонны при срыве  насосов (не более 8 тн), без учета веса '
         f'штанг в  0,9т. При отрицательном результате согласов технологической службой ЦДНГ или ПТО региона  постепенное увеличение нагрузки до 15тн ( по 1т - 1 час),  либо искусственный  отворот НШ с последующим комбинированным подъемом ГНО НВ. В случае невозможности отворота колонны НШ с подтверждением супервайзера, распиловку НШ согласовать с ПТО по направлению сектора учета НКТ и НШ.',
         None, None, None, None, None, None, None,
         'Мастер КРС представитель Заказчика, пусков. Ком. ',
    ...
    lift_pump_nv_with_paker = [
        ...
        [None, None,
         ...
         f'{"".join([" " if krs.without_damping(self) == True else f"Приподнять штангу. Произвести глушение в НКТ в объеме{volume_pod_NKT()}м3. Техостой 2ч."])}'
         f'Поднять на штангах насос с гл. {int(CreatePZ.dict_pump_h["do"])}м с доливом тех жидкости уд.весом {fluid_work}г/см3 Обеспечить не превышение расчетных нагрузок на штанговые колонны при срыве  насосов (не более 8 тн), без учета веса '
         f'штанг в  0,9т. При отрицательном результате согласов технологической службой ЦДНГ или ПТО региона  постепенное увеличение нагрузки до 15тн ( по 1т - 1 час),  либо искусственный  отворот НШ с последующим комбинированным подъемом ГНО НВ. В случае невозможности отворота колонны НШ с подтверждением супервайзера, распиловку НШ согласовать с ПТО по направлению сектора учета НКТ и НШ.',
         None, None, None, None, None, None, None,
         'Мастер КРС представитель Заказчика, пусков. Ком. ',
         round((0.17 + 0.015 * sum(list(CreatePZ.dict_nkt.values())) / 8.5 + 0.12 + 1.02), 1)],
    ...
    lift_pump_nn_with_paker = [
        ...
        [None, None,
         ...
         f'{"".join([" " if krs.without_damping(self) == True else f"Приподнять штангу. Произвести глушение в НКТ в объеме{volume_pod_NKT()}м3. Техостой 2ч."])}'
         f' Поднять на штангах насос {CreatePZ.dict_pump["do"]}с гл. {int(CreatePZ.dict_pump_h["do"])}м с доливом тех жидкости уд.весом {fluid_work}г/см3 '
         f'Обеспечить не превышение расчетных нагрузок на штанговые колонны при срыве  насосов (не более 8 тн), без учета веса '
         f'штанг в  0,9т. При отрицательном результате согласов технологической службой ЦДНГ или ПТО региона  постепенное увеличение нагрузки до 15тн ( по 1т - 1 час),  либо искусственный  отворот НШ с последующим комбинированным подъемом ГНО НВ. В случае невозможности отворота колонны НШ с подтверждением супервайзера, распиловку НШ согласовать с ПТО по направлению сектора учета НКТ и НШ.',
         None, None, None, None, None, None, None,
         'Мастер КРС представитель Заказчика, пусков. Ком. ',
         round((0.17 + 0.015 * int(CreatePZ.dict_pump_h["do"]) / 8.5 + 0.12 + 1.02), 1)],
    ...
    lift_ecn_with_paker = [
        ...
        [None, None,
         f'Сбить сбивной клапан. '
         f'{"".join([" " if krs.without_damping(self) == True else f"Произвести глушение в НКТ в объеме{volume_pod_NKT()}м3. Техостой 2ч."])}',
         None, None,
         None, None, None, None, None,
         'Мастер КРС представитель Заказчика, пусков. Ком. ', 4.2],
    ...
    lift_ord = [
        ...
        [None, None,
         ...
         f'инженером от 14.10.2021г при СПО штанг (ПМШ 62х21 либо аналог). Опрессовать ПВО на 40атм. '
         f'{"".join([" " if krs.without_damping(self) == True else f"Приподнять штангу. Произвести глушение в НКТ в объеме{volume_pod_NKT()}м3. Техостой 2ч."])}'
         f'Поднять на штангах насос с гл. {int(CreatePZ.dict_pump_h["do"])}м с доливом тех жидкости уд.весом {fluid_work}г/см3 '
         f'Обеспечить не превышение расчетных нагрузок на штанговые колонны при срыве  насосов (не более 8 тн), без учета веса '
         f'штанг в  0,9т. При отрицательном результате согласов технологической службой ЦДНГ или ПТО региона  постепенное увеличение нагрузки до 15тн ( по 1т - 1 час),  либо искусственный  отворот НШ с последующим комбинированным подъемом ГНО НВ. В случае невозможности отворота колонны НШ с подтверждением супервайзера, распиловку НШ согласовать с ПТО по направлению сектора учета НКТ и НШ.',
         None, None, None, None, None, None, None,
         'Мастер КРС представитель Заказчика, пусков. Ком. ',
    ...
    if ('ЭЦН' in CreatePZ.dict_pump['do'].upper() or 'ВНН' in CreatePZ.dict_pump['do'].upper()) and (
        ...
        for row in without_damping(self).sorted(reversed = True):
            lift_ord.insert(4, row)
        lift_select['ОРД'] = lift_ord

        print(f'Подьем орд')
    elif 'ЭЦН' in CreatePZ.dict_pump['do'].upper() or 'ВНН' in CreatePZ.dict_pump['do'].upper() and str(CreatePZ.paker_do[
        'do']) == '0':
        for row in without_damping(self).sorted(reversed = True):
            lift_ecn.insert(2, row)
        lift_select['ЭЦН'] = lift_ecn
        ...
        for row in without_damping(self).sorted(reversed = True):
            lift_ecn_with_paker.insert(5, row)
        lift_select['ЭЦН с пакером'] = lift_ecn_with_paker
        print('Подьем ЭЦН с пакером ')

    elif 'НВ' in CreatePZ.dict_pump['do'].upper() and CreatePZ.if_None(CreatePZ.paker_do['do']) == 'отсут':
        for row in without_damping(self).sorted(reversed = True):
            lift_pump_nv.insert(1, row)
        lift_select['НВ'] = lift_pump_nv
        print('Подьем НВ')
    elif 'НВ' in CreatePZ.dict_pump['do'].upper() and CreatePZ.if_None(CreatePZ.paker_do['do']) != 'отсут':
        for row in without_damping(self).sorted(reversed = True):
            lift_pump_nv_with_paker.insert(5, row)
        lift_select['НВ с пакером'] = lift_pump_nv_with_paker
        print('Подьем НВ с пакером ')
    elif 'НН' in CreatePZ.dict_pump['do'].upper() and CreatePZ.if_None(CreatePZ.paker_do['do']) == 'отсут':
        for row in without_damping(self).sorted(reversed = True):
            lift_pump_nn.insert(1, row)
        lift_select['НН'] = lift_pump_nn
        print('Подьем НН')
    elif 'НН' in CreatePZ.dict_pump['do'].upper() and CreatePZ.if_None(CreatePZ.paker_do['do']) != 'отсут':
        for row in without_damping(self).sorted(reversed = True):
            lift_pump_nn_with_paker.insert(5, row)
        lift_select['НН с пакером'] = lift_pump_nn_with_paker
        print('Подьем НН с пакером')

    elif str(CreatePZ.dict_pump['do']) == '0' and CreatePZ.if_None(CreatePZ.paker_do['do']) == 'отсут':
        for row in without_damping(self).sorted(reversed = True):
            lift_voronka.insert(0, row)
        lift_select['воронка'] = lift_voronka
        print('Подьем  воронки')
    elif str(CreatePZ.dict_pump['do']) == '0' and CreatePZ.if_None(CreatePZ.paker_do['do']) != 'отсут':
        for row in without_damping(self).sorted(reversed = True):
            lift_paker.insert(5, row)
        lift_select['пакер'] = lift_paker
        print('Подьем пакера')
    elif 89 in CreatePZ.dict_nkt.keys() and 48 in CreatePZ.dict_nkt.keys() and CreatePZ.if_None(CreatePZ.paker_do['do']) != 'отсут':
        for row in without_damping(self).sorted(reversed = True):
            lift_orz.insert(5, row)
        lift_select['ОРЗ'] = lift_orz
        print('Подьем ОРЗ')

    lift_select_quest = QMessageBox.question(self, 'Подъем ГНО', f'Программа определила что в скважине спущено {lift_select.keys()}. верно ли?')
    if lift_select_quest == QMessageBox.StandardButton.Yes:
    ...
    return krs_begin + list(lift_select.values()) + posle_lift
...
def well_jamming(self, without_damping, lift_select):
    from open_pz import CreatePZ
    # {'пакер': lift_paker, 'ОРЗ': lift_orz, 'ОРД': lift_ord, 'Воронка': lift_voronka,
    #  'НН с пакером': lift_pump_nn_with_paker, 'НВ с пакером': lift_pump_nv_with_paker,
    #  'ЭЦН с пакером': lift_ecn_with_paker, 'ЭЦН': lift_ecn, 'НВ': lift_pump_nv, 'НН': lift_pump_nn}
    if without_damping == True:
        well_jamming_list = [[None, None,
             f'Скважина состоит в перечне скважин ООО "Башнефть-Добыча", '
              f'на которых допускается проведение ТКРС без предварительного глушения на текущий квартал',
              None, None, None, None, None, None, None,
              ' Мастер КРС', None
              ],
            [None, None,
             f'В случае наличия избыточного давления необходимость повторного глушения скважины дополнительно согласовать со специалистами ПТО  и ЦДНГ.',
             None, None, None, None, None, None, None,
             ' Мастер КРС', None]]
    elif without_damping  == False and lift_select in ['ОРД', 'воронка', 'НН с пакером', 'НВ с пакером',
        'ЭЦН с пакером', 'ЭЦН', 'НВ', 'НН']:

        # 'пакер', 'ОРЗ', 'ОРД', 'воронка', 'НН с пакером', 'НВ с пакером',
        #         'ЭЦН с пакером', 'ЭЦН', 'НВ', 'НН'
        well_jamming_list = [[None, None,
                              f'Произвести глушение скважины прямой промывкой тех жидкостью уд.весом {CreatePZ.fluid_work} на циркуляцию в объеме '
