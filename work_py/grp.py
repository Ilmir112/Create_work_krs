...
class GrpWindow(WindowUnion):
    ...
    def grpPaker(self, diameter_paker, paker_depth, paker_khost, gis_otz_true_quest, gis_otz_after_true_quest,
        ...
        paker_list = [
            ...
            [None, None,
             f'Опрессовать глухие плашки превентора на максимально ожидаемое давление на устье '
             f'{self.data_well.max_expected_pressure.get_value}атм, но не выше '
        ...
        if gis_otz_true_quest == 'Да':
            paker_list.append(
                [f'Отбить забой по ГК и ЛМ', None,
                 f'Вызвать геофизическую партию. Заявку оформить за 16 часов сутки через РИТС {data_list.contractor}". '
                 f'Произвести  монтаж ПАРТИИ ГИС согласно схемы  №11 утвержденной главным инженером  {data_list.DICT_CONTRACTOR[data_list.contractor]["Дата ПВО"]}г. '
