...
class GnoParent(ABC):
    def __init__(self, parent=None):
        ...
        self.nv_list = ["Допустить фНКТ для определения текущего забоя. "
                        if self.data_well.open_trunk_well is False and
    ...
    def lifting_unit(self):
        ...
        upa_60 = 'Установить подъёмный агрегат на устье не менее 60т.  \n' \
                 ...
                 'ветровых оттяжек до 50 тн. при скорости ветра менее 14 м/с и плотности грунта не менее 5 кг см2. ' \
                 'При выполнении работ по ликвидации  аварии обязательно оснащение всех видов ПОДЪЕМНЫХ АГРЕГАТОВ 4 ' \
                 'ОТТЯЖКАМИ. \nПри превышении 75% от страгивающей нагрузки на все подъемные агрегаты оформляется ' \
                 'наряд допуск на проведение работ повышенной опасности. МАКСИМАЛЬНУЮ НАГРУЗКА НЕ ДОЛЖНА ПРЕВЫШАТЬ ' \
                 '80% ОТ СТРАГИВАЮЩЕЙ НАГРУЗКИ НА НКТ. После монтажа подъёмника якоря ветровых оттяжек должны ' \
                 'быть испытаны на нагрузки, установленные инструкцией по эксплуатации завода - изготовителя в ' \
                 'присутствии супервайзера Заказчика. '
        text = upa_60 if self.data_well.bottom_hole_artificial.get_value >= 2300 else aprs_40
        return text + '\nВо время монтажа/демонтажа подъемного агрегата определять опасную зону размером ' \
                      'периметра 50 х 50 ' \
                      'метров вокруг ремонтируемой скважины. В случае отсутствия оттяжек размеры опасной зоны - ' \
                      'высота мачты подъемного агрегата +10 м). \nВъезд/выезд на территорию опасной зоны обозначить' \
                      ' предупредительным знаком «СТОЙ опасная зона» или светоотражающей лентой, преграждающими свободный ' \
                      'въезд. Во время монтажа/демонтажа подъемного агрегата запретить нахождение и передвижение ' \
                      'транспортных средств в опасной зоне.\n' \
                      'В процессе монтажа ветровых оттяжек подъемного агрегата обеспечить установку на них ' \
    ...
    def _krs_begin_work_base_rows(self, surfactant_hydrofabizer_str: str) -> list:
        ...
        return [
            ...
            [None, 'п/п', 'Наименование работ', None, None, None, None, None, None, None,
             ...
             'Нормы времени \n мин/час.'],
            [None, None,
             f'Движение транспортных средств по территории ремонтируемой скважины допускается только под руководством '
             f'бурильщика бригады, выполняющего функцию регулировщика. Регулировка движения в темное время '
             f'осуществляется световым жезлом.',
             None, None, None, None, None, None, None,
            ...
            [None, 3,
             ...
             f'ознакомлена с возможными осложнениями и авариями в процессе работ, планом локализации и ликвидации '
    ...
    def begin_work(self):
        ...
        if 1 in сat_h2s_list or 2 in сat_h2s_list:
            krs_begin.extend([
                [None, None, f'Перед работой на скважинах, содержащих сероводород, обеспечить прокачку коллекторных '
                             f'линий жидкостью глушения с добавлением нейтрализатора сероводорода, информацию о '
                             f'прокачке включить в акт глушения скважины. Демонтаж коллекторной линии на скважинах, '
    ...
    def well_jamming(self):
        ...
        well_jamming_list2 = f'Вести контроль плотности на  выходе в конце глушения. В случае ' \
                             ...
                             f'{"А ТАКЖЕ ПРИ ГАЗОВОМ ФАКТОРЕ БОЛЕЕ 200м3/сут " if self.data_well.gaz_factor_percent[0] > 200 else ""} ' \
        ...
        if self.without_damping_true:
            ...
            well_jamming_str = f'Произвести глушение скважины объеме {self.volume_well_jamming}м3 тех ' \
                               ...
                               f'(согласовать глушение в коллектор, в случае ' \
    ...
    def lifting_paker(self):
        ...
        lift_paker = [
            ...
            [f'Произвести срыв пакера не более {round(weigth_pipe(self.data_well.dict_nkt_before) * 1.2, 1)}т. '
             ...
             f'нагрузки до 28т (страгивающую нагрузка НКТ по паспорту),  при необходимости '
             ...
             f' с момента расхаживания- выйти с согласование на УСРСиСТ, ПТО Региона - '
        ...
        if '89' in list(map(str, self.data_well.dict_nkt_before.keys())) and '48' in list(
            ...
            lift_orz = [
                ...
                [f'Сорвать планшайбу и пакер', None,
                 ...
                 f'По истечению 30 минут, после срыва пакера, произвести проверку свободного хода инструмента'
                 f'(не менее 5 раз). Путем подьема на полный "вылет" НКТ, поэтапно, по 2 метра. \nПРИМЕЧАНИЕ: '
                 f'При отрицательном результате согласовать с УСРСиСТ ступенчатое увеличение '
                 f'нагрузки до 28т ( страгивающая нагрузка НКТ по паспорту),  при необходимости  с '
                 ...
                 f' с момента расхаживания - выйти с согласование на УСРСиСТ, ПТО Региона - для составления '
                 f'алгоритма'
                 f' последующих работ. ', None, None,
                 None, None, None, None, None,
    ...
    def lifting_ord(self):
        lift_ord = [
            ...
            [None, None,
             ...
             None, None, None, None, None, None, None,
             'Мастер КРС представитель Заказчика, пусков. Ком. ', 1.2],
            [f'подъем {self.data_well.dict_pump_shgn["before"]}', None,
             ...
             f'штанг в  0,9т. При отрицательном результате согласовать технологической службой ЦДНГ или ПТО '
            ...
            [f'Сорвать планшайбу и пакер  не более {round(weigth_pipe(self.data_well.dict_nkt_before) * 1.2, 1)}т. '
             ...
             f'(вес подвески ({round(weigth_pipe(self.data_well.dict_nkt_before), 1)}т) + 20%). '
             f'По истечению 30 минут, после срыва пакера, произвести проверку свободного хода инструмента(не менее 5 раз). Путем подьема на полный "вылет" НКТ, поэтапно, по 2 метра. \n'
             f'ПРИМЕЧАНИЕ: При отрицательном '
             f'результате согласовать с УСРСиСТ ступенчатое увеличение '
             f'нагрузки до 28т (страгивающую нагрузка НКТ по паспорту),  при необходимости  с '
             f'противодавлением в НКТ '
             f'(время на прибытие СТП ЦА 320 + АЦ не более 4 часов). Общие время на расхаживание - не более 6 '
             f'часов, через 5 часов '
             f' с момента расхаживания - выйти с согласование на УСРСиСТ, ПТО Региона -  для составления '
    ...
    def lifting_voronka(self):
        lift_voronka = [
            ...
            [f'Сорвать планшайбу не более {round(weigth_pipe(self.data_well.dict_nkt_before) * 1.2, 1)}т. '
             ...
             f'(вес подвески ({round(weigth_pipe(self.data_well.dict_nkt_before), 1)}т) + 20%). ПРИМЕЧАНИЕ: '
             f'При отрицательном результате согласовать с УСРСиСТ ступенчатое увеличение '
             f'нагрузки до 28т (страгивающую нагрузка НКТ по паспорту),  при '
             ...
             f' с момента расхаживания- выйти с согласование на УСРСиСТ, ПТО '
    ...
    def lifting_nn_with_paker(self):
        ...
        lift_pump_nn_with_paker = [
            ...
            ['Поднять плунжер', None,
             ...
             f'Обеспечить не превышение расчетных нагрузок на штанговые колонны при срыве '
             f'насосов (не более 8 тн), без учета веса '
             f'штанг в  0,9т. При отрицательном результате согласовать технологической службой ЦДНГ или ПТО региона '
    ...
    def lifting_nv_with_paker(self):
        lift_pump_nv_with_paker = [
            ...
            [f'Сорвать планшайбу и пакер не более {round(weigth_pipe(self.data_well.dict_nkt_before) * 1.2, 1)}т. '
             ...
             f'(вес подвески ({round(weigth_pipe(self.data_well.dict_nkt_before), 1)}т) + 20%).  '
             f'По истечению 30 минут, после срыва пакера, произвести проверку свободного хода '
             f'инструмента(не менее 5 раз). Путем подьема на полный "вылет" НКТ, поэтапно, по 2 метра. \n'
             f'ПРИМЕЧАНИЕ: При отрицательном '
             f'результате согласовать с УСРСиСТ ступенчатое увеличение '
             f'нагрузки до 28т (страгивающую нагрузка НКТ по паспорту),  при необходимости  с '
             f'противодавлением в НКТ '
    ...
    def lifting_ecn_with_paker(self):
        ...
        lift_ecn_with_paker = [
            ...
            [f'срыв пакера не более {round(weigth_pipe(self.data_well.dict_nkt_before) * 1.2, 1)}т. '
             ...
             f'(время на прибытие СТП ЦА 320 +  АЦ не более 4 часов). Общие время на расхаживание - '
             f'не более 6 часов, '
             f'через 5 часов с момента расхаживания- выйти с согласование на УСРСиСТ, ПТО Региона '
    ...
    def lifting_ecn(self):
        lift_ecn = [
            ...
            [f'Сбить сбивной клапан. {self.well_jamming_str[2]}', None,
             f'При герметичности: Сбить сбивной клапан. При негерметичности согласовать подъем '
             f'до "зеркала". \n{self.well_jamming_str[0]}',
             None, None, None, None, None, None, None,
    ...
    def lifting_with2_paker(self):
        lift_ecn_with_2paker = [
            [f'Опрессовать ГНО на Р=50атм', None,
             ...
             'Мастер КРС представитель Заказчика ', 0.7],
            [f'Сбить сбивной клапан. {self.well_jamming_str[2]}', None,
             f'При герметичности: Сбить сбивной клапан. При негерметичности согласовать подьем до "зеркала". {self.well_jamming_str[0]}',
             None, None, None, None, None, None, None,
             'Мастер КРС, представ заказчика', 3.2],
            [None, None, self.well_jamming_str[1],
             None, None, None, None, None, None, None,
