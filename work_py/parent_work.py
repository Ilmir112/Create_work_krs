...
class TabPageUnion(QWidget):
    ...
    def update_skv_edit(self, index):
        ...
        if index == 'без СКВ':
            ...
            self.skv_acid_edit.setVisible(False)
            self.skv_volume_label.setVisible(False)
            self.skv_volume_edit.setVisible(False)
            self.skv_proc_label.setVisible(False)
            self.skv_proc_edit.setVisible(False)
        else:
            self.skv_acid_label_type.setVisible(True)
            self.skv_acid_edit.setVisible(True)
            self.skv_volume_label.setVisible(True)
            self.skv_volume_edit.setVisible(True)
            self.skv_proc_label.setVisible(True)
...
class WindowUnion(MyMainWindow):
    ...
    def pvo_gno(self, kat_pvo):
        ...
        pvo_2 = f'Установить ПВО по схеме №2 утвержденной главным инженером {contractor} {date_str} (тип плашечный ' \
                ...
                f'Спустить пакер на глубину 10м. Опрессовать ПВО (трубные плашки превентора) на Р-{self.text_pvo},' \
                f' сорвать пакер. '
