...
class FindIndexPZ(MyMainWindow):
    ...
    def __init__(self, ws, work_plan, parent=None):
        ...
        (
            self.column_direction_mine_diameter,
            self.column_direction_mine_wall_thickness,
        ...
        self.check_data_in_pz = [
            "Рассмотрев план заказ были выявлены следующие нарушения, "
    ...
    def delete_rows_pz(self, ws, cat_well_min, data_well_max, data_x_max):
        ...
        row_heights1 = [
            ws.row_dimensions[i + 1].height
            for i in range(cat_well_min.get_value, ws.max_row)
        ]
        ...
        for key, value in boundaries_dict.items():
            if (
                ...
                ws.merge_cells(
                    start_column=value[0],
                    start_row=value[1] + 16 - cat_well_min.get_value + 1,
                    end_column=value[2],
                    end_row=value[3] + 16 - cat_well_min.get_value + 1,
                )
    ...
    def read_pz(self):
        ...
        for row_ind, row in enumerate(
            ...
            if self.cat_well_min.get_value != 0:
                if self.image_loader:
                    coord_image = list(
                        map(
                            lambda x: int("".join([y for y in x if y.isdigit()])),
                            list(self.image_loader._images.keys()),
                        )
                    )
        ...
        if self.data_x_max.get_value == 0:
            QMessageBox.warning(
                self,
                "Ошибка",
                "Не корректный файл excel, либо отсутствует строка с "
        ...
        if self.data_well_min.get_value == 0:
            QMessageBox.warning(
                self,
                "индекс начала строки после план заказ",
        ...
        if self.sucker_rod_none:
            if self.sucker_rod_ind.get_value == 0:
                sucker_mes = QMessageBox.question(
                    self,
                    "ШТАНГИ",
                    "Программа определелила, что в скважине "
                ...
                if sucker_mes == QMessageBox.StandardButton.Yes:
                    ...
                    QMessageBox.information(
                        self, "ШТАНГИ", 'Нужно добавить "ШТАНГИ" в таблицу?'
                    )
                    # self.pause_app()
                    return
        ...
        if self.data_x_min.get_value == 0:
            QMessageBox.warning(
                self,
                "индекс начала копирования ожидаемых показателей",
        ...
        if self.data_fond_min.get_value == 0:
            QMessageBox.warning(
                self,
                "индекс начала строки с таблицей фондового оборудования",
        ...
        if self.type_kr == "":
            QMessageBox.information(
                self,
                "Вид ГТМ",
                "Приложение не смогло найти тип КР, " "необходимо внести вручную",
            )
        ...
        if self.bcu_level is False:
            QMessageBox.warning(
                self,
                "безопасный статический уровень",
    ...
    def read_pz_prs(self):
        ...
        if any([self.ws.cell(row=i, column=1).value != None for i in range(1, 20)]):
            QMessageBox.warning(
                self,
                "Ошибка",
                'Для корректной работы приложения нужно сместить колонку "А" в право',
            )
            # self.pause_app()
            return
        ...
        for row_ind, row in enumerate(
            ...
            if "Категория скважины" in row:
                ...
                self.ws.cell(row=row_ind + 1, column=2).value = (
                    "XIa ТЕХНИЧЕСКОЕ ЗАДАНИЕ от ЦДНГ:"
                )
        ...
        if self.data_x_max.get_value == 0:
            QMessageBox.warning(
                self,
                "индекс окончания копирования ожидаемых показателей",
        ...
        for col in range(1, 12):
            ...
            if image_loader.image_in(coord):
                ...
                if image_file:
                    ...
                    )  # Замените 'PNG' на другой формат, если необходимо
    ...
    def read_work_data(self):
        ...
        if need_depth and need_depth > self.current_bottom:
            if rir_roof_pvr < need_depth and rir_roof_pvr != 0:
                self.need_depth = self.current_bottom
            else:
                self.need_depth = need_depth
        elif self.current_bottom > self.perforation_sole and need_depth is None:
    ...
    def definition_is_none(self, data, row, col, step, m=12):
        if data.__class__ == ProtectedIsNonNone:
            ...
            data = data.get_value
            while (data is None) and (step < m):
                data = self.check_once_isdigit(
                    self.ws.cell(row=row, column=col + step).value
                )
                step += 1
...
def insert_column_direction(text):
    try:
        ...
        try:
            diameter = ProtectedIsDigit(column_direction_data[0].replace(" ", ""))
        except Exception as e:
            diameter = ProtectedIsNonNone(f"не корректно {e}")

        try:
            wall_thickness = ProtectedIsDigit(column_direction_data[1].replace(" ", ""))
        except Exception:
            wall_thickness = ProtectedIsNonNone("не корректно")
        try:
            try:
                length = ProtectedIsDigit(
                    column_direction_data[2]
                    .split("-")[1]
                    .replace("(м)", "")
                    .replace(" ", "")
                )
        ...
        for row_ind, row in enumerate(
            ...
            if row_ind >= 1:
                ...
                if key != str(None) and key != "-" and "диам" not in key.lower():
                    ...
                    if value:
                        if row_ind < a_plan:
                            dict_nkt[key] = dict_nkt.get(key, 0) + round(
                                self.check_str_none(value), 1
                            )
                        elif row_ind >= a_plan:
                            dict_nkt_po[key] = dict_nkt_po.get(key, 0) + round(
                                self.check_str_none(value), 1
                            )
...
class WellSuckerRod(FindIndexPZ):
    ...
    def read_well(self, begin_index, cancel_index):
        ...
        if self.sucker_rod_ind.get_value != 0:
            ...
            if b_plan == 0:
                sucker_rod_question = QMessageBox.question(
                    self,
                    "отсутствие штанг",
                    "Программа определило что штанг в " "скважине нет, корректно?",
                )
            ...
            for row_ind, row in enumerate(
                ...
                if row_ind >= 1:
                    ...
                    if (
                        ...
                        if key is not None and row_ind < b_plan:
                            try:
                                ...
                                QMessageBox.warning(
                                    self,
                                    "Ошибка",
                                    "Ошибка в определении длины штанг до ремонта, "
                        ...
                        if key is not None and row_ind >= b_plan:
                            try:
                                dict_sucker_rod_po[key] = dict_sucker_rod_po.get(
                                    key, 0
                                ) + int(float(str(value).replace(",", ".")))
                            except Exception:
                                QMessageBox.warning(
                ...
                if value:
                    ...
                    if "Пакер" in str(value) and "типоразмер" in str(row[col + 2]):
                        ...
                        if dict_pump_ecn["after"] != 0:
                            dict_pump_ecn_h["after"] = self.check_str_none(
                                self.ws.cell(
                        ...
                        try:
                            if paker_do["before"] != 0:
                                depth_fond_paker_do["before"] = self.check_str_none(
                                    row[col_do]
                                )[0]
                                depth_fond_paker2_do["before"] = self.check_str_none(
                                    row[col_do]
                                )[1]
                        except Exception:
                            if paker_do["before"] != 0:
                        ...
                        try:
                            if paker_do["after"] != 0:
                                depth_fond_paker_do["after"] = self.check_str_none(
                                    row[col_plan]
                                )[0]
                                depth_fond_paker2_do["after"] = self.check_str_none(
                                    row[col_plan]
                                )[1]
                        except Exception as e:
                            if paker_do["after"] != 0:
...
class WellHistoryData(FindIndexPZ):
    ...
    def read_well(self, begin_index, cancel_index):
                ...
                if value:
                    ...
                    if "Начало бурения" in str(value):
                        ...
                        if type(self.date_commissioning.get_value) is datetime:
                            self.date_commissioning = ProtectedIsNonNone(
                                self.date_commissioning.get_value.strftime("%d.%m.%Y")
                            )
                        ...
                        self.result_pressure = self.definition_is_none(
                            self.result_pressure, row_index + begin_index, col + 1, 1
                        )
                        ...
                        if type(self.result_pressure_date.get_value) is datetime:
                            self.result_pressure_date = ProtectedIsDigit(
                                self.result_pressure_date.get_value.strftime("%d.%m.%Y")
                            )
        ...
        if self.max_admissible_pressure.get_value in ["", 0, "0"]:
            self.check_data_in_pz.append(
                "не указано максимально допустимое давление на устье\n"
            )
        if float(self.max_admissible_pressure.get_value) < 30:
            ...
            self.check_data_in_pz.append(
                f"максимально допустимое давление на устье слишком маленькое"
...
class WellCondition(FindIndexPZ):
    ...
    def read_well(self, begin_index, cancel_index):
            ...
            for col, cell in enumerate(row):
                ...
                if value is not None:
                    ...
                    if value:
                        if "Hст " in str(value):
                            if "/" in str(row[col + 1]):
                                self.static_level = ProtectedIsDigit(
                                    row[col + 1].split("/")[0]
                                )
                            else:
                            ...
                            if self.check_str_isdigit(well_volume_in_pz) is False:
                                well_volume_in_pz, _ = QInputDialog.getDouble(
                                    self,
                                    "Объем глушения",
                            ...
                            self.well_volume_in_pz.append(
                                round(float(well_volume_in_pz), 1)
                            )
                            ...
                            if self.check_str_isdigit(well_fluid_in_pz):
                                self.well_fluid_in_pz.append(
                                    round(float(well_fluid_in_pz), 2)
                                )
                            else:
                                QMessageBox.warning(
                                    self,
                                    "Ошибка",
                                    f"Не корректно прочитан удельный "
        ...
        if self.leakiness_count != 0:
            leakiness_quest = QMessageBox.question(
                self,
                "нарушение колонны",
            ...
            for col, cell in enumerate(row[0:15]):
                ...
                if value:
                    ...
                    if "прием" in str(value).lower() or "qж" in str(value).lower():
                        ...
                        self.expected_pickup = self.definition_is_none(
                            self.expected_pickup, row_index, col + 1, 1
                        )
                    ...
                    if (
                        ...
                        self.expected_pressure = self.definition_is_none(
                            self.expected_pressure, row_index, col + 1, 1
                        )
                    ...
                    if "qж" in str(value).lower():
                        ...
                        self.water_cut = self.definition_is_none(
                            self.water_cut, row_index, col + 1, 1
                        )

                    if "qн" in str(value).lower():
                        self.expected_oil = (
                            str(row[col + 1]).replace(" ", "").replace("т/сут", "")
                        )
                        self.expected_oil = self.definition_is_none(
                            self.expected_oil, row_index, col + 1, 1
                        )
                    if "воды" in str(value).lower() and "%" in str(value).lower():
                        try:
                            proc_water = (
                                str(row[col + 1]).replace(" ", "").replace("%", "")
                            )

                            proc_water = self.definition_is_none(
                                proc_water, row_index, col + 1, 1
                            )
                            self.percent_water = (
                                int(float(proc_water))
                                if float(proc_water) > 1
                                else round(float(proc_water) * 100, 0)
                            )
                        except Exception:
    ...
    def read_well(self, begin_index, cancel_index):
            ...
            for col, cell in enumerate(row):
                ...
                if value:
                    ...
                    if "площадь" in str(value):
                        if self.work_plan not in ["prs"]:
                            self.well_number = ProtectedIsNonNone(
                                str(row[col - 1]).lstrip().rstrip()
                            )
                        self.well_area = ProtectedIsNonNone(
                            str(row[col + 1]).lstrip().rstrip().replace(" ", "_")
                        )
        ...
        if self.cdng.get_value not in list(dict_data_cdng.keys()):
            QMessageBox.warning(
                self,
                "Ошибка",
                f"ЦДНГ - {self.cdng.get_value} отсутствует в списках, "
...
class WellData(FindIndexPZ):
    ...
    def read_well(self, begin_index, cancel_index):
                ...
                if value:
                    if (
                        ...
                        self.bottom_hole_drill = self.definition_is_none(
                            self.bottom_hole_drill, row_index, col, 2
                        )

                        self.bottom_hole_artificial = ProtectedIsDigit(row[col + 4])

                        self.bottom_hole_artificial = self.definition_is_none(
                            self.bottom_hole_artificial, row_index, col, 5
                        )
                    ...
                    if "иск. забой" in str(value).lower():
                        ...
                        self.max_angle = ProtectedIsDigit(
                            self.check_once_isdigit(row[col + 4])
                        )
                        ...
                        self.current_bottom = FindIndexPZ.definition_is_none(
                        ...
                        if (
                            ...
                            column_direction_mine_data = FindIndexPZ.definition_is_none(
                                self, column_direction_mine_data, row_index, col, 2
                            )
                            (
                                self.column_direction_mine_diameter,
                                self.column_direction_mine_wall_thickness,
                                self.column_direction_mine_length,
                            ) = insert_column_direction(column_direction_mine_data)
                            self.level_cement_direction_mine = ProtectedIsNonNone(
                                row[col + 9]
                            )
                        else:
                            self.column_direction_mine_true = False
                        ...
                        if self.column_direction_true:
                            ...
                            column_direction_data = FindIndexPZ.definition_is_none(
                                self, column_direction_data, row_index, col, 2
                            )
                            (
                                self.column_direction_diameter,
                                self.column_direction_wall_thickness,
                                self.column_direction_length,
                        ...
                        if self.column_conductor_true:
                            ...
                            column_conductor_data = FindIndexPZ.definition_is_none(
                                self, column_conductor_data, row_index, col, 2
                            )
                            (
                                self.column_conductor_diameter,
                                self.column_conductor_wall_thickness,
                                self.column_conductor_length,
                            ) = insert_column_direction(column_conductor_data)
                        ...
                        for col1, cell in enumerate(row):
                            if "Уровень цемента" in str(cell):
                                try:
                                    ...
                                    self.level_cement_conductor = ProtectedIsNonNone(
                                        "не корректно"
                                    )
                        try:
                            ...
                            self.column_conductor_diameter = ProtectedIsNonNone(
                                "не корректно"
                            )
                            self.column_conductor_wall_thickness = ProtectedIsNonNone(
                                "не корректно"
                            )
                            self.column_conductor_length = ProtectedIsNonNone(
                                "не корректно"
                            )
                    elif self.check_text_in_row(
                        ...
                        (
                            ...
                            self.column_wall_thickness,
                            self.shoe_column,
                        ) = insert_column_direction(data_main_production_string)
                        ...
                        self.level_cement_column = ProtectedIsDigit(
                            str(row[col + 2]).strip()
                        )
                        ...
                        data_column_additional = self.check_str_none(
                            self.ws.cell(row=row_index + 2, column=col + 2).value
                        )
                        ...
                        if self.column_additional is True:
                            try:
                                ...
                                self.head_column_additional = ProtectedIsNonNone(
                                    "не корректно"
                                )
                            try:
                                ...
                                self.shoe_column_additional = ProtectedIsNonNone(
                                    "не корректно"
                                )
                            ...
                            try:
                                ...
                                self.column_additional_diameter = ProtectedIsNonNone(
                                    "не корректно"
                                )
                        else:
                            self.column_additional_diameter = ProtectedIsNonNone(
        ...
        try:
            ...
            if self.dict_pump_shgn["before"] not in [
                "0",
                0,
            ] and self.dict_pump_shgn_depth["before"] not in ["0", 0]:
                adwdr = abs(
                ...
                if self.dict_sucker_rod:
                    if (
                        ...
                        QMessageBox.warning(
                            self,
                            "Ошибка",
                            f"Длина штанг {sum(list(self.dict_sucker_rod.values()))}м "
            ...
            if self.distance_from_well_to_sampling_point == 0:
                QMessageBox.warning(
                    self, "Ошибка", f"Не указано расстояние до пункта налива"
                )
                self.check_data_in_pz.append(f"Не указано расстояние до пункта налива")

            if self.dict_pump_shgn["after"] not in [
                "0",
                0,
            ...
            if self.max_angle.get_value > 45 or "gnkt" in self.work_plan:
                angle_true_question = QMessageBox.question(
                    self,
                    "Зенитный угол",
            ...
            if str(self.well_number.get_value) in [
                ...
                QMessageBox.warning(
                    self,
                    "Канатные технологии",
                    f"Скважина согласована на канатные технологии",
                )
            ...
            if "0" != str(self.dict_pump_ecn["after"]):
                if sum(list((self.dict_nkt_after.values()))) - self.dict_pump_ecn_depth[
                    "after"
                ] < -10 and "48" not in list(self.dict_nkt_before.keys()):
                    QMessageBox.warning(
                        self,
                        "Ошибка",
                        f"Длина НКТ {sum(list(self.dict_nkt_after.values()))}м "
    ...
    def read_angle_well():
        ...
        while fname is None:
            fname, _ = QtWidgets.QFileDialog.getOpenFileName(
                None, "Выберите файл", ".", "Файлы Exсel (*.xlsx);;Файлы Exсel (*.xls)"
            )
            ...
            if (
                ...
                mes = QMessageBox.question(
                    None,
                    "Ошибка",
                    "Файл не корректный, необходимо загрузить корректный файл",
                )
                if mes == QMessageBox.StandardButton.No:
                    fname = "без загрузки"
                else:
        ...
        if fname not in ["без загрузки"]:
            ...
            for index_row, row in enumerate(
                ...
                if (
                    ...
                    angle_data.append(
                        (
                            float(str(row[depth_column]).replace(",", ".")),
                ...
                if (
                    ...
                    if self.check_str_isdigit(row[col_vert_index]):
                        self.dict_perforation.setdefault(plast, {}).setdefault(
                            "вертикаль", []
                        ).append(float(str(row[col_vert_index]).replace(",", ".")))
                    ...
                    sole_int = round(
                        float(str(row[col_sole_index]).replace(",", ".")), 1
                    )
                    self.dict_perforation.setdefault(plast, {}).setdefault(
                        "интервал", []
                    ).append((roof_int, sole_int))
                    self.dict_perforation_short.setdefault(plast, {}).setdefault(
                        "интервал", []
                    ).append((roof_int, sole_int))
                    ...
                    if self.old_version is False:
                        if row[col_close_index] is None or row[col_close_index] == "-":
                            ...
                            self.dict_perforation_short.setdefault(
                                plast, {}
                            ).setdefault("отключение", False)
                        else:
                            self.dict_perforation.setdefault(plast, {}).setdefault(
                                "отключение", True
                            )
                    ...
                    if "давление" in self.dict_perforation[plast]:
                        self.dict_perforation[plast]["давление"] = [
                    ...
                    if row[col_date_pressure_index]:
                        self.dict_perforation.setdefault(plast, {}).setdefault(
                            "замер", []
                        ).append(row[col_date_pressure_index])
                    else:
                        self.dict_perforation.setdefault(plast, {}).setdefault(
                            "замер", []
                        ).append(0)
                    ...
                    self.dict_perforation.setdefault(plast, {}).setdefault(
                        ...
                        calculation_fluid_work(
                            self, row[col_vert_index], row[col_pressure_index]
                        )
                    )
                    ...
                    if row[col_vert_index] is not None:
                        self.dict_perforation_project.setdefault(plast, {}).setdefault(
                            "вертикаль", []
                        ).append(
                    ...
                    if row[col_pressure_index] is not None:
                        self.dict_perforation_project.setdefault(plast, {}).setdefault(
                            "давление", []
                        ).append(round(self.check_str_none(row[col_pressure_index]), 1))
                    self.dict_perforation_project.setdefault(plast, {}).setdefault(
                        "рабочая жидкость", []
                    ).append(
                        calculation_fluid_work(
            ...
            for plast, value in self.dict_perforation.items():
                ...
                for roof_int, sole_int in sorted(list(intervals), key=lambda x: x[0]):
                    ...
                    if not merged_segments or roof_int > merged_segments[-1][1]:
                        ...
                        merged_segments[-1] = [
                            merged_segments[-1][0],
                            max(sole_int, merged_segments[-1][1]),
                        ]
            ...
            try:
                if (
                    ...
                    bsu_data = int(
                        ...
                        - float(max(data["давление"]))
                        * 100
                        / max(data["рабочая жидкость"])
                        / 9.81
                    )
                    self.dict_perforation.setdefault(plast, {}).setdefault(
                        "БСУ", bsu_data
    ...
    def read_well(self, begin_index, cancel_index):
        ...
        if data_list.data_in_base is False:
            try:
                    ...
                    for col in range(1, 13):
                        ...
                        if cell:
                            if (
                                ...
                                self.category_pressure_well.append(
                                ...
                                self.category_gaz_factor_percent.append(
                                    self.ws.cell(row=row, column=col - 2).value
                                )
                                if (
                                        "отс"
                                    ...
                                    self.gaz_factor_percent.append(
                                        float(
                                            str(
                                                self.ws.cell(
                                                    row=row, column=col - 1
                ...
                QMessageBox.warning(
                    self, "Ошибка", f"Ошибка обработки данных по категориям {e}"
                )
            ...
            if len(self.category_h2s_list) == 0:
                QMessageBox.warning(
                    self,
                    "ОШИБКА",
                    "Приложение не смогла найти значение " "сероводорода в процентах",
                )
                data_list.pause = True
                # self.pause_app()
                return
            ...
            if len(self.value_h2s_percent) == 0:
                QMessageBox.warning(
                    self,
                    "Ошибка",
                    "Программа не смогла найти данные по содержания "
                    "сероводорода в процентах",
                )
                h2s_pr, _ = QInputDialog.getDouble(
                    self,
                    "сероводород в процентах",
                    "Введите значение сероводорода в процентах",
                    0,
                    0,
                    100,
                    5,
        ...
        for plast in self.dict_perforation:
            if self.dict_perforation[plast]["отключение"] is False:
                zamer_str = None
                zamer = list(
                    filter(lambda x: x != 0, self.dict_perforation[plast]["замер"])
                )
                if zamer:
                    zamer = zamer[0]
