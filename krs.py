    ...
    if work_plan != 'dop_plan':
        ...
        lift_ecn_with_paker = [
            ...
            [None, None,
             ...
             f'{"".join([" " if without_damping_True == True else f"При наличии Избыточного давления не позволяющее сорвать пакера: Произвести глушение в НКТ в объеме {volume_pod_NKT()}м3. {fluid_work}"])}'
           ...
           [None, None,
            ...
            [f'Поднять  {CreatePZ.dict_pump_ECN["do"]} с пакером {CreatePZ.paker_do["do"]}', None,
             ...
             f'колпачков с доливом скважины тех.жидкостью уд. весом {fluid_work}  '
        ...
        lift_pump_nv = [
            ...
            [f'Поднять {CreatePZ.dict_pump_SHGN["do"]} с гл. {int(CreatePZ.dict_pump_SHGN_h["do"])}м', None,
             ...
             f'{CreatePZ.max_expected_pressure}атм. Поднять на штангах насос '
        ...
        lift_pump_nv_with_paker = [
            ...
            [f'Поднять насос {CreatePZ.dict_pump_SHGN["do"]}', None,
             ...
             f'{"".join([" " if without_damping_True == True else f"При наличии Избыточного давления не позволяющее сорвать пакера: Приподнять штангу. Произвести глушение в НКТ в объеме{volume_pod_NKT()}м3. Техостой 2ч."])}'
             f' Поднять на штангах насос с гл. {float(CreatePZ.dict_pump_SHGN_h["do"])}м с '
             f'доливом тех жидкости уд.весом {fluid_work} '
        ...
        lift_pump_nn = [
            ...
            [f'поднять плунжен', None,
             ...
             f'инженером от 14.10.2021г при СПО штанг (ПМШ 62х21 либо аналог). Опрессовать ПВО на '
             f'{CreatePZ.max_expected_pressure}атм. Заловить конус спуском одной '
             f'штанги. Поднять на штангах плунжер с гл. {float(CreatePZ.dict_pump_SHGN_h["do"])}м с доливом тех '
             f'жидкости уд.весом {fluid_work} '
             f'Обеспечить не превышение расчетных нагрузок на штанговые колонны при срыве  насосов (не более 8 тн), без учета веса '
             f'штанг в  0,9т. При отрицательном результате согласов технологической службой ЦДНГ или ПТО региона  '
             f'постепенное увеличение нагрузки до 15тн ( по 1т - 1 час), либо искусственный  отворот НШ с последующим '
        ...
        lift_pump_nn_with_paker = [
            ...
            ['Поднять плунжер', None,
             ...
             f'инженером от 14.10.2021г при СПО штанг (ПМШ 62х21 либо аналог). Опрессовать ПВО на {CreatePZ.max_expected_pressure}атм. Спуском одной штанги заловить конус. '
             f'{"".join([" " if without_damping_True == True else f"При наличии Избыточного давления не позволяющее сорвать пакера: Приподнять штангу. Произвести глушение в НКТ в объеме{volume_pod_NKT()}м3. Техостой 2ч."])}'
           ...
           [None, None,
            ...
            [f'Поднять  насос {CreatePZ.dict_pump_SHGN["do"]} с пакером {CreatePZ.paker_do["do"]}',
             ...
             f'замером, накручиванием колпачков с доливом скважины тех.жидкостью уд. весом {fluid_work}  '
             f'в объеме {round(round(sum(list(CreatePZ.dict_nkt.values())), 1) * 1.12 / 1000, 1)}м3 с контролем АСПО '
             f'на стенках НКТ.',
             None, None,
             None, None, None, None, None,
        ...
        lift_voronka = [[well_jamming_str[0], None, well_jamming_str[0],
                        ...
                        [f'Поднять воронку  с Н-{round(sum(list(CreatePZ.dict_nkt.values())), 1)}м',
                         ...
                         f'доливом скважины тех.жидкостью уд. весом {fluid_work}  '
        ...
        lift_paker = [[f'Опрессовать эксплуатационную колонну и пакер на Р={CreatePZ.max_admissible_pressure}атм',
                      ...
                      [None, None,
                       f'{lifting_unit(self)}', None, None, None, None, None, None, None,
                       'Мастер КРС представитель Заказчика, пусков. Ком. ', 4.2],
                      ['Произвести срыв пакера не более {round(weigth_pipe(CreatePZ.dict_nkt) * 1.2, 1)}т. '
                       f'(вес подвески ({round(weigth_pipe(CreatePZ.dict_nkt), 1)}т) + 20%)', None,
        ...
        if 89 in list(map(int, CreatePZ.dict_nkt.keys())) and 48 in list(map(int, CreatePZ.dict_nkt.keys())):
            ...
            lift_orz = [
                ...
                [well_jamming_str[2], None,
                 ...
                 [str(well_jamming_norm(volume_pod_NKT())) if without_damping_True is False else None][0]],
                [None, None,
                 well_jamming_str[1],
                 None, None, None, None, None, None, None,
                 ' Мастер КРС',
                ...
                [f'Поднять компоновку ОРЗ с глубины {CreatePZ.dict_nkt["89"]}м', None,
                 ...
                 f'накручиванием колпачков с доливом скважины тех.жидкостью уд. весом {fluid_work}  '
...
def pvo_gno(kat_pvo):
    from open_pz import CreatePZ
    # print(f' ПВО {kat_pvo}')
    pvo_2 = f'Установить ПВО по схеме №2 утвержденной главным инженером ООО "Ойл-сервис" от 14.10.2021г (тип плашечный ' \
            ...
            f'задвижек на Р-{CreatePZ.max_expected_pressure}атм  (на максимально ожидаемое давление ' \
            f'на устье, но не выше максимально допустимого давления опрессовки эксплуатационной колонны в течении ' \
    ...
    pvo_1 = f'Установить ПВО  по  схеме №1 утвержденной главным инженером ООО "Ойл-сервис" от 14.10.2021г ' \
            ...
            f'блока дросселирования -БД 65х21 (при необходимости произвести монтаж переводника П178х168 или П168 х 146 или ' \
            f'П178 х 146 в зависимости от типоразмера крестовины и колонной головки). Линия и блок-глушения БГ 65х21 находится на ' \
            f'базе БПО, если расстояние от БПО до ремонтируемой скважины составляет менее 150км. Спустить и посадить ' \
            ...
            f'материалом механические задвижки манифольдные линии до блока дросселирования\n' \
            ...
            f'Получить разрешение на производство работ в присутствии представителя ПФС'
    if kat_pvo == 1:
        return pvo_1, f'Монтаж ПВО по схеме №1'
    else:
        # print(pvo_2)
